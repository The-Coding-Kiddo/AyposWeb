...
const Home = () => {
  ...
  const [migrationModel, setMigrationModel] = useState<string>('ssl');
  ...
  const timeOptions = ['1', '5', '20'];
  const stepOptions = ['3', ...Array.from({ length: 30 }, (_, i) => ((i + 1) * 5).toString())];
  const modelTypes = ['lstm'];
  const migrationModelTypes = {
    direct: ['ssl'],
    indirect: ['xgboost', 'ann']
  };
  const migrationMethodTypes = ['mathematical', 'AI'];

  // Inside the Home component, add new state
  const [estimationMethod, setEstimationMethod] = useState<'direct' | 'indirect'>('direct');
